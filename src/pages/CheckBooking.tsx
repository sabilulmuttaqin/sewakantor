...
import { BookingDetails } from "../types/require";
...
export default function CheckBooking() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    console.log("Valid data, Submitting... ", formData);
    setLoading(true);

    try {
      const response = await axios.post(
        "http://127.0.0.1:8000/api/booking-details",
        {
          ...formData,
        },
        {
      ...
      console.log("Were check your booking: ", response.data.data);
      ...
      >
        ...
        >
          ...
          <button
            disabled={loading}
            type="submit"
            className="flex items-center justify-center rounded-full p-[12px_30px] gap-3 bg-primary font-bold text-[#F7F7FD]"
          >
        ...
        {/* {error && (
          <div className="flex items-center justify-center w-full py-6 bg-[#FFEBE8] rounded-[20px] border border-[#FF2D2D]">
            <p className="font-semibold text-[#FF2D2D]">
              Booking tidak ditemukan. Silakan periksa kembali ID dan nomor
              telepon Anda.
            </p>
          </div>
        )} */}
              ...
              <div className="flex flex-col gap-5">
                {bookingDetails.is_paid ? (
                  <div className="flex items-center justify-between">
                    ...
                    <p className="rounded-full w-fit p-[6px_16px] bg-primary font-bold text-sm leading-[21px] text-[#F7F7FD]">
                      SUCCESS
                    </p>
                  </div>
                ) : (
